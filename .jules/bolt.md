...
**Learning:** Beyond caching static pointers, per-frame IL2CPP method calls for positions (`get_position`) and on-screen projections (`WorldToScreenPoint`) are major bottlenecks when processing 100+ entities. Staggered updates and plane-culling are essential for maintaining high FPS.
**Action:** Use a global frame counter to stagger position refreshes for static/semi-static entities (e.g., Gear every 60 frames, Harvestables every 180). Implement a fast C++ dot-product plane check before calling expensive IL2CPP projection methods. Avoid using member functions on POD structs like `Unity::Vector3` unless explicitly defined; use manual component math instead.
